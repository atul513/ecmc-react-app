...
import {
    apiGetPracticeSets, apiDeletePracticeSet, apiPublishPracticeSet,
...
const DescCell = ({ html }) => {
    ...
    return (
        <div className="text-xs text-gray-400 mt-0.5 max-w-xs">
            ...
            {isLong && (
                <button type="button" className="ml-1 text-primary underline text-xs"
...
const statusBadge = (status) => {
    const map = { draft: 'bg-yellow-100 text-yellow-700', published: 'bg-emerald-100 text-emerald-700' }
    return <span className={`text-xs px-2 py-1 rounded-full font-medium ${map[status] ?? 'bg-gray-100 text-gray-500'}`}>{status}</span>
}
...
const PracticeSetList = () => {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            toast.push(<Notification type="danger" title="Failed to load practice sets" />, { placement: 'top-center' })
        } finally {
            setLoading(false)
        }
    }, [filters, search])
    ...
    const handlePublish = async (item) => {
        ...
        try {
            ...
            toast.push(<Notification type="success" title="Published successfully" />, { placement: 'top-center' })
            load()
        } catch {
    ...
    const confirmDelete = async () => {
        try {
            ...
            toast.push(<Notification type="success" title="Deleted" />, { placement: 'top-center' })
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        <h3 className="text-lg font-semibold">
                            Practice Sets {total > 0 && <span className="text-sm font-normal text-gray-400">({total})</span>}
                        </h3>
                        <Button variant="solid" icon={<TbPlus />} onClick={() => navigate(`${ECMC_PREFIX_PATH}/practice/create`)}>
                            Create Practice Set
                        </Button>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                        <Input prefix={<TbSearch />} placeholder="Search..." value={search}
                            onChange={(e) => { setSearch(e.target.value); setFilters((f) => ({ ...f, page: 1 })) }}
                            className="w-48" />
                        <Select options={STATUS_OPTIONS} value={STATUS_OPTIONS.find((o) => o.value === filters.status)}
                            onChange={(opt) => setFilter('status', opt?.value ?? '')} className="w-36" placeholder="Status" />
                        <Select options={ACCESS_OPTIONS} value={ACCESS_OPTIONS.find((o) => o.value === filters.access_type)}
                            onChange={(opt) => setFilter('access_type', opt?.value ?? '')} className="w-36" placeholder="Access" />
                        <Select options={categoryOptions} value={categoryOptions.find((o) => o.value === filters.category_id)}
                            onChange={(opt) => setFilter('category_id', opt?.value ?? '')} className="w-44" placeholder="Category" />
                        <Select options={subjectOptions} value={subjectOptions.find((o) => o.value === filters.subject_id)}
                            onChange={(opt) => setFilter('subject_id', opt?.value ?? '')} className="w-40" placeholder="Subject" />
                    </div>
                        ...
                        <>
                            <div className="overflow-x-auto -mx-4 sm:mx-0">
                                <div className="min-w-[700px] px-4 sm:px-0">
                                    <Table>
                                        ...
                                        <TBody>
                                            {sets.length === 0 ? (
                                                <Tr><Td colSpan={7} className="text-center text-gray-400 py-8">No practice sets found</Td></Tr>
                                            ) : sets.map((s) => (
                                                <Tr key={s.id}>
                                                    <Td>
                                                        <div className="font-medium">{s.title}</div>
                                                        <DescCell html={s.description} />
                                                    </Td>
                                                    <Td className="text-sm">{s.subject?.name ?? '—'}</Td>
                                                    <Td className="text-sm">{s.questions_count ?? s.total_questions ?? '—'}</Td>
                                                    <Td>
                                                        <span className={`text-xs px-2 py-1 rounded-full font-medium ${s.access_type === 'paid' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-500'}`}>
                                                            {s.access_type}
                                                        </span>
                                                    </Td>
                                                    <Td>
                            ...
                            {total > filters.per_page && (
                                <div className="flex justify-end mt-2">
                                    <Pagination total={total} pageSize={filters.per_page} currentPage={filters.page}
