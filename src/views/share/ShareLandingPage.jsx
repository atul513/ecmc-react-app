...
import {
    ...
    TbUserPlus, TbCalendar, TbClock, TbAlertCircle,
    TbMoodSad,
} from 'react-icons/tb'
...
const ShareLandingPage = () => {
    ...
    useEffect(() => {
        ...
        apiResolveShareLink(shareCode, src)
            .then((res) => {
                ...
                if (d?.valid === false) {
                    setExpired(true)
                    setErrorMsg(d.message || 'This link has expired or is no longer active.')
                } else {
                    setData(d)
                }
            })
            .catch((err) => {
                ...
                if (status === 410 || status === 404) {
                    setExpired(true)
                    setErrorMsg(err?.response?.data?.message || 'This link has expired or is no longer active.')
                } else {
    ...
    const handleStart = () => {
        if (authenticated) {
            ...
            if (type === 'quiz' || type === 'exam') navigate(`/quiz/${data.content.id}`)
            else if (type === 'practice_set') navigate(`/practice-sets/${data.content.id}`)
            else if (type === 'coding_test') navigate(`/coding-tests/${data.content.id}`)
            else navigate('/home')
        } else {
    ...
    const ContentIcon = data?.content?.type ? (CONTENT_ICONS[data.content.type.toLowerCase()] || TbClipboardList) : TbClipboardList
            ...
            <div className="max-w-xl mx-auto px-4 pt-32 pb-20 flex flex-col items-center">
                {loading ? (
                    ...
                    <div className="w-full text-center py-16">
                        ...
                        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-3">Link Expired</h1>
                    ...
                    <div className="w-full">
                        ...
                        {(data.share?.title || data.share?.message) && (
                            <div className="bg-primary/5 dark:bg-primary/10 border border-primary/20 rounded-2xl px-5 py-4 mb-5 text-center">
                                {data.share?.title && (
                                    <p className="font-semibold text-primary text-sm mb-1">{data.share.title}</p>
                                )}
                                {data.share?.message && (
                                    <p className="text-gray-600 dark:text-gray-400 text-sm">{data.share.message}</p>
                                )}
                            </div>
                        )}
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
                            ...
                            {/* Meta */}
                            {(data.content?.total_questions || data.content?.total_duration_min) && (
                                <div className="flex items-center justify-center gap-5 px-6 py-3 bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
                                    {data.content?.total_questions != null && (
                                        <div className="flex items-center gap-1.5 text-sm text-gray-500">
                                            ...
                                            {data.content.total_questions} questions
                                        </div>
                                    )}
                                    {data.content?.total_duration_min != null && (
                                        <div className="flex items-center gap-1.5 text-sm text-gray-500">
                                            <TbClock size={15} />
                                            {data.content.total_duration_min} min
                                        </div>
                                    )}
                                </div>
                            )}
                                ...
                                {authenticated ? (
                                    ...
                                    <>
                                        <Button
                                            ...
                                            className="w-full"
                                        >
                                            {data.require_registration ? 'Register to Start' : 'Sign In to Start'}
                                        </Button>
                                        <p className="text-xs text-gray-400">
                                            Already have an account?{' '}
                                            <Link to={`/sign-in?redirect=/share/${shareCode}`} className="text-primary hover:underline">
                                                Sign in
                                            </Link>
                                        </p>
                                    </>
                        ...
                        <p className="text-center text-xs text-gray-300 dark:text-gray-600 mt-6">
                            Powered by ECMC Learning Platform
                        </p>
                    </div>
                ) : (
